...
func (t *Tools) Slugify(s string) (string, error) {
	...
	if len(slug) == 0 {
		return "", errors.New("after slugifying a string, there are no characters left")
	}

	return slug, nil
}

// DownloadStaticFile downloads a file and tries to force the browser to avoid displaying it in the
// browser window by setting content disposition. It also allows specification od the diplay name
...
// ReadJSON tries to read the body of a request and converts from json to go variables
func (t *Tools) ReadJSON(w http.ResponseWriter, r *http.Request, data interface{}) error {
	...
	if err != nil {
		...
		case errors.As(err, &syntaxError):
			return fmt.Errorf("body contains malformed JSON at charcter %d", syntaxError.Offset)
		case errors.Is(err, io.ErrUnexpectedEOF):
			return errors.New("body contains malformed JSON")
		case errors.As(err, &unmarshalTypeError):
			if unmarshalTypeError.Field != "" {
				return fmt.Errorf("body contains unknown JSON type for field %q", unmarshalTypeError.Field)
			}
...
// WriteJSON takes a response status code ad arbitrary data abd writes json to the client
...
// PushJSONToRemote posts arbitrary data to some uri as JSON, and returns the response, status code and error, if any
